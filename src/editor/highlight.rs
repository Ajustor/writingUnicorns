...
fn push_normal(tokens: &mut Vec<Token>, text: &str) {
    if !text.is_empty() {
        tokens.push(Token { text: text.to_string(), kind: TokenKind::Normal });
    }
}
...
fn generic_tokenize(
    ...
    while i < len {
        ...
        if line[char_byte_offset(&chars, i)..].starts_with(comment_prefix) {
            tokens.push(Token { text: line[char_byte_offset(&chars, i)..].to_string(), kind: TokenKind::Comment });
            return tokens;
        }
...
fn tokenize_rust(line: &str) -> Vec<Token> {
    ...
    for tok in &mut tokens {
        ...
        if tok.kind == TokenKind::Normal || tok.kind == TokenKind::Function {
            if end < src_chars.len() && src_chars[end] == '!' {
...
fn tokenize_js_ts(line: &str) -> Vec<Token> {
    generic_tokenize(
        ...
        &["abstract","arguments","as","async","await","break","case","catch","class",
          ...
          "undefined","var","void","while","with","yield","declare","keyof","infer",
          "never","unknown","any","override","satisfies","accessor"],
        &["boolean","number","string","symbol","bigint","object","Array","Promise",
          "Record","Partial","Required","Readonly","Pick","Omit","Exclude","Extract",
          "NonNullable","ReturnType","InstanceType","Map","Set","Date","RegExp","Error"],
    )
}
...
fn tokenize_python(line: &str) -> Vec<Token> {
    ...
    generic_tokenize(
        ...
        &["int","float","str","bool","bytes","list","dict","set","tuple","type",
          "object","super","property","classmethod","staticmethod","Exception",
...
fn tokenize_toml(line: &str) -> Vec<Token> {
    ...
    if trimmed.starts_with('[') {
        return vec![Token { text: line.to_string(), kind: TokenKind::KeywordType }];
    }
...
fn tokenize_shell(line: &str) -> Vec<Token> {
    ...
    generic_tokenize(
        ...
        &["if","then","else","elif","fi","for","while","do","done","case","esac",
          "function","return","exit","export","local","readonly","source",
          "echo","cd","ls","mkdir","rm","cp","mv","cat","grep","sed","awk","find",
          "chmod","chown","sudo","true","false","in"],
        &[],
    )
}
...
fn tokenize_dockerfile(line: &str) -> Vec<Token> {
    ...
    for instr in &instructions {
        if upper == *instr || upper.starts_with(&format!("{} ", instr)) || upper.starts_with(&format!("{}\t", instr)) {
            let instr_len = instr.len();
            // Find leading whitespace length
            let leading = line.len() - trimmed.len();
            let mut tokens = Vec::new();
            if leading > 0 {
                tokens.push(Token { text: line[..leading].to_string(), kind: TokenKind::Normal });
            }
            tokens.push(Token { text: line[leading..leading + instr_len].to_string(), kind: TokenKind::Keyword });
